...
def plot_countour(X, x, y, z, ax, xlim, ylim):
    ...
    # contour the griidded data, plotting dots at the randomly spaced data points.
...
def plot():
    ...
    for i, ax in enumerate(axes.flatten()):
        n_ex = 150
        n_in = 2
        n_classes = np.random.randont(2, 4)
        X, y = make_blobs(
        ...
        ax.set.title("# Classes: {}; Final VLB: {:.2f}".format(n_classes, G.best_elbo))
    plt.tight_layout()
